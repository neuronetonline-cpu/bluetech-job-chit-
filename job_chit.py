    ...
    def print_click():
        ...
        if messagebox.askyesno('Print Job Chit','Send the job chit to your DEFAULT Windows printer?',parent=win):
            try:
                ...
                # Windows may not have a PDF application's "print" verb registered.
                # Keep the one-page PDF unchanged and open it for manual printing.
                try:
                    os.startfile(path)
                    messagebox.showinfo(
                        'Printer',
                        'Direct printing is not available on this PC.\n\n'
